<div class="col-md-2 pt-3" style="border-left: 1px solid #e0e0e0; background-color: #f3f2f0;">
	...
	<a class="card p-2 mb-2 border-yellow" href="{{ url('t')}}/{{ $user->username }}">
		<div class="row">
		  	<div class="col-4 my-auto">
		      @if($user->image == null)
		        <img class="img-fluid rounded-circle" style="max-height: 50px;" src="{{ url('images/profile_picture') }}/default-profile-picture.png">
		      @else
		        <img class="img-fluid rounded-circle" style="max-height: 50px;" src="{{ url('images/profile_picture') }}/{{ $user->image }}">
		      @endif
		    </div>
		    <div class="col-6 text-yellow font-weight-bold my-auto" >{{ $user->name }}</div>
